...
プレイヤー1（青・先手）とプレイヤー2（赤）が、6個ずつの駒を交互に1マス動かします。移動できる方向は移動プロファイルと現在の戦闘力から決まり、標準設定では一部の戦闘力で1方向が使用できなくなります。敵駒のマスへ移動すると互いの戦闘力を同時にダメージとして与え合い、戦闘力が0以下になった駒は消滅します。
...
主なUnity PackageはURP `17.3.0`、Input System `1.19.0`、uGUI `2.0.0`、Test Framework `1.6.0`です。
...
5. 選択中の駒で「パワーランダム化」を押すと、通常戦闘力を1〜3へ変更して手番を消費します。
6. 隣接する自駒がいる場合は「合体」を押し、青く示されたもう一方の自駒を選ぶと合体を試行します。
7. 左上のBGM／SFXスライダーで音量を調節できます。
8. ゲーム終了後は「スタート画面に戻る」でタイトルへ戻れます。初期状態へ戻す場合は右上の「リセット」を押します。
