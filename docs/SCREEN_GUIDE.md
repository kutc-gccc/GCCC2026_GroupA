...
|---|---|
| 左上 | 現在の手番と▲▼ |
| 中央 | 盤面。白い数字は一時効果を含む現在の戦闘力 |
| 左中央 | 点・枠・特殊マスの凡例 |
| 右上 | リザーブ配置、合体、パワーランダム化 |
| 右側 | 上が▼、下が▲のリザーブ一覧。`駒 n / 6`は盤上とリザーブを合わせた所有数 |
| 左下 | BGM／SFX音量 |
| 右下 | リセット。選択やリザーブも含めてゲームを初期状態へ戻す |

### 基本操作

左クリック、またはプライマリタッチで押します。
...
以下は撮影時の説明文の省略・誤解しやすい箇所です。今回の文書更新ではゲーム内文言やルールを変更していません。

| 画面の表現 | 現行実装では |
|---|---|
| 第1節「上＝相手、下＝自分」 | 先手▲の視点。後手▼の相手陣地は下 |
| 第3節「3つのうち1つだけ」 | 選択した盤上駒の行動を説明している。リザーブ配置も別途1手の行動になる |
| 第4節「動ける向きが永久に減る」 | `ProfileMoveDirectionResolver`は現在の`EffectiveCombatPower`で再計算する。戦闘力が下がれば方向も対応する帯域へ戻る |
| 第5節「序盤の戦闘は必ず相打ち」 | 初期値1同士なら相打ち。序盤でもランダム化・合体・特殊マスで数字が変われば結果は異なる |
| 第6節の合体説明 | 合体成功済みの駒は再合体できない。確率失敗した駒は再試行できる |
| 第6節「リザーブを1つもらえる」 | 所有数が上限6個の場合は獲得しない。現行実装はその場合も効果適用履歴を残すため、後から同じ駒で踏み直しても獲得できない |
