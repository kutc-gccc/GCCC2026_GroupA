...
シアンは止まっている間だけの効果、紫は止まるたび何度でもリザーブ駒を獲得できる効果です。リザーブは右側の一覧から選び、白い点の空きマスへ置きます。上限や合体制限の詳細は[ゲームルール §8](GAME_RULES.md#8-パワーランダム化合体特殊マス)を参照してください。
...
手番の下に、直前の1行動の結果を表示します。次の行動まで残り、駒の選択や遊び方の開閉では消えません。長い結果は欄内をホイールまたはドラッグで縦スクロールできます。配置案内をキャンセルすると直前の結果へ戻り、リセットで消去します。

| 結果例 | 意味 |
|---|---|
| `戦闘 2対1`、`攻撃側：残り1 ／ 防御側：消滅` | 実際の戦闘値と双方の結果 |
| `攻撃前に強化解除：3→1` | マスを離れたため、表示されていた強化分は今回の攻撃に使われない |
| `戦闘力 2→2（変化なし・手番消費）` | ランダム化は同じ数値になっても1手 |
| `戦闘力＋2：1→3（このマスにいる間）` | 停止先の特殊効果で一時的に強化された |
| `リザーブ＋1（止まるたびに獲得）` | 停止先の効果による獲得 |
| `リザーブ獲得なし：所持上限 6/6` | 移動は成立したが、所有上限で今回は増えなかった |

戦闘と停止先の効果は同じ行動の結果にまとめます。上のゲーム画面画像は結果欄拡張前の配置であり、遊び方ボタンの位置と結果欄の高さは現行画面と異なります。
