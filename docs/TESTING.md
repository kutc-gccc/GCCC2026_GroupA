...
- リザーブ獲得マスを同じ駒が何度でも獲得できること、上限で見送っても空きが戻れば再び獲得できること
- リザーブ追加、所有駒上限6、両プレイヤーの配置範囲、リセット、合体時の効果履歴継承
- 適用済み通知と上限による未獲得通知、効果単位のEvent順序、複数効果での一部獲得、上限変更とEveryStop再進入（`GameSessionFeedbackTests`）
...
- `ActionResultMessageTests`：戦闘の双方生存パターンと相打ち、強化失効、ランダム化の増減・同値、合体、特殊マスのLifetime、複数効果と一部獲得、配置案内からの結果復元、Command拒否時の保持
- `BoardGameActionLogTests`：本番Bootstrapの効果ログ、選択と遊び方開閉での保持、リセット、長文スクロール・新結果での先頭復帰・入力遮断・UIの重なり
...
- [ ] 戦闘・ランダム化・特殊マスの結果が直前の行動として残り、上限による未獲得と強化解除の理由も読める
- [ ] 1920×1080で結果欄と遊び方ボタンが重ならず、長文はスクロールで最後まで読める。結果欄操作で駒や手番が変わらない
